############ EX-UI Installation Script ############
# Authors: Antonio Steiger antonio.steiger@tum.de
#
# Last Updated: 8.11.22
# Purpose:  Prepare computer for execution of start.py and any EX-UI
...
def linux_handleRootTasks(GFX, PKGMGR):
    ...
    installcmds += "EXUIPWCRYPT=$(perl -e 'print crypt($ARGV[0], \"salt\")' " + EXUIPW + ")\n"
    installcmds += "id -u " + EXUIUSERNAME + " || sudo useradd -s /bin/bash -d /home/exui/ -m -p ${EXUIPWCRYPT} -G sudo exui\n" #TODO: Remove sudo rights if they turn out to be unnecessary
    ...
    if(PKGMGR == "apt"):
        ...
        if linux_isCMDAvailable("docker") == False:
            ...
            installcmds += "sudo usermod -aG docker " + EXUIUSERNAME + "\n"
...
def linux_pullDockerRegistry():
    process = sh.Popen(["docker", "pull", "registry:latest"])
    process.wait()
...
if __name__ == "__main__":
    # Entry Point
    _print("###### EX-UI Installation Script ######")
