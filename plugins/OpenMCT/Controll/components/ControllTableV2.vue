...
export default {
    ...
    inject: ['openmct', 'objectPath', 'ControllTableV2'],
    props: {
        isEditing: Boolean
    },
    data() {
        return {
            domainObject: this.ControllTableV2,
            newFrameLocation: [],
            identifierMap: {},
            SocketPort : this.ControllTableV2.adapter,
            pswd : this.ControllTableV2.paswd,
            Name: this.ControllTableV2.name
        };
    },
    computed: {
        layoutDirectionStr() {
    ...
    methods: {
        ...
        createSocket(){
            ...
            var url = protocol + domain + ":" + this.SocketPort;
            ...
            Socket.addEventListener("open", (e) => {
                ...
                Socket.addEventListener("close", here.websocketClosedHandler);
                Socket.addEventListener("error", here.websocketErrorHandler);
                Socket.addEventListener("message", here.websocketMessageHandler);

                // Send keepalives every 1 s (browsers limit intervals to 1 s for inactive
                // tabs, so any faster doesn't make sense)
                here.sendKeepalive();
                clearInterval(socketInterval);
                ...
                socketTimeoutObj = setTimeout(() => {
                    console.log("WS: timeout during operation, reconnecting...")
                    here.createSocket();
                }, timeout);
            });
        },
        ...
        websocketErrorHandler() {
            console.log(Socket.url + "WS: error, closing socket and try reconnecting")
        },
        async handleClick(){
            var DATA = {
                commands: []
            ...
            var promises = this.domainObject.composition.map(function(obj){
                return openmct.objects.get(obj).then(async function (ControllOB){
                    if (ControllOB.transition != 2) {
                        DATA.commands.push({ cmd: "set", key: ControllOB.command, value: ControllOB.transition})
                    }

                    if (ControllOB.type == "ControllButtonV2"){
